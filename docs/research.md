...
- **Publication:** Cheng et al. (Peking University & DeepSeek-AI), arXiv preprint
- **Date:** January 2026
- **Summary:** Introduces Engram, a pretraining-time conditional-memory module placed at specific Transformer layers alongside the MoE + Attention stack, which retrieves static N-gram embeddings via deterministic hashing. Under matched parameters and FLOPs, the paper finds a U-shaped sparsity-allocation law: allocating ~70–80% of the parameter budget to experts and ~20–30% to Engram beats pure MoE. A 100B-parameter Engram table can be offloaded to host memory at <3% throughput cost because lookups are deterministic and prefetchable.
...
Cheng, Xin, et al. "Conditional Memory via Scalable Lookup: A New Axis of Sparsity for Large Language Models." *arXiv*, 12 Jan. 2026, arxiv.org/abs/2601.07372.
