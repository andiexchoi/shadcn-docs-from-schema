// Ablation: prompt.js with the platformGuidelines module (Apple HIG, Material
// Design, ARIA patterns) stripped. Framing philosophy, formatting rules,
// template, output budget, style-guide, and semantic-guidelines all stay.
//
// Tests whether the specific a11y wins (motion-reduce, aria-describedby,
// explicit ARIA wiring) trace back to platformGuidelines. If the semantic
// tier collapses under this ablation, platformGuidelines is carrying that
// work. If not, the effect lives elsewhere (likely semantic-guidelines or
// the upstream shadcn docs themselves).

import { semanticGuidelines } from "../../../src/semantic-guidelines.js";
import { styleGuide } from "../../../src/style-guide.js";

function buildSharedPromptBodyNoPlatformGuidelines(componentIdentifier) {
...
[One sentence: what this component does and when you would reach for it. Be specific.]
...
**Don't**
- [Only include a Don't when the negative framing adds something the positive version cannot capture. Start with "Don't" or "Avoid." Follow with the positive alternative when possible.]
...
[Required sub-components and their nesting rules. Composition patterns: which elements must wrap which. Required props or attributes that the underlying primitive enforces even if TypeScript allows omitting them. asChild and ref forwarding requirements.]

## Variants and options

[For each prop that changes the component's appearance or behavior, explain what it is for and when you would choose it. Skip internal implementation props like className, asChild, ref, onOpenChange. Bold the variant name, then explain in 1 to 3 sentences.]
...
[Explicit key-to-action mapping for this component. Format as a list: key or key combination, then what it does. Include focus trap behavior and focus return on close.]

## ARIA requirements

[Specific ARIA attributes this component requires. Name the attribute, the element it goes on, and what value it takes. Do not list generic ARIA advice.]

## Accessibility

[Focus management, screen reader announcements, motion considerations, contrast requirements, and any accessibility behavior not covered in the Keyboard interactions or ARIA requirements sections.]

## Common mistakes

[Implementation errors specific to this component. Focus on mistakes an AI coding agent or a developer unfamiliar with the component library would make. State what goes wrong, then state the correct approach.]
...
export function buildPromptFromDocsNoPlatformGuidelines(componentName, docsContent) {
...
${buildSharedPromptBodyNoPlatformGuidelines(componentName)}
