...
/* ─────────────────────────────────────────
   Local Dialog wrappers (no shadcn dialog)
───────────────────────────────────────── */
...
export interface ConfirmDeleteDialogProps {
  ...
  onCancel?: () => void;
  /**
   * Control the open state externally. When omitted the dialog manages
   * its own open/close state via the trigger button.
   */
  open?: boolean;
  onOpenChange?: (open: boolean) => void;
  /**
   * Render a custom trigger. When omitted a default "Delete project" button
   * is rendered.
   */
  trigger?: React.ReactNode;
  /** Show a loading spinner on the delete button while the async action runs. */
  isDeleting?: boolean;
}
...
export function ConfirmDeleteDialog({
  ...
  const handleOpenChange = React.useCallback(
    (next: boolean) => {
      if (!next) {
        // Reset input whenever dialog closes
        setInputValue("");
        onCancel?.();
      }
      if (isControlled) {
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            with <strong className="text-foreground font-medium">{projectName}</strong> will
            be deleted forever.
          </DialogDescription>
        </DialogHeader>
