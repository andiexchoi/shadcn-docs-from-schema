...
This study addresses that gap for one narrow scope: the effect on Claude Sonnet 4.6's code when a CLAUDE.md generated from the canonical shadcn/ui and Radix documentation is injected into the system prompt for ticket-style component-generation prompts. The tool producing the CLAUDE.md is documented elsewhere in this repository; the prompt pipeline is summarized in Section 4.3. The hypothesis under test is that CLAUDE.md injection raises the fraction of pre-registered guidelines satisfied by the generated `.tsx` output.

The null hypothesis is no systematic shift in marker satisfaction between conditions.
